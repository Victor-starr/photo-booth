                  ...
                  {session.photo_urls.slice(0, 4).map((url, idx) => {
                    ...
                    return (
                      <Image
                        ...
                        width={140}
                        height={140}
                        className="absolute shadow-md border-4 border-gray-300 rounded-lg object-cover hover:scale-110 transition-transform duration-300"
                        style={style}
                        unoptimized
                      />
                    );
  ...
  return (
    <div className="gap-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 w-full max-w-5xl">
      {Array.from({ length: 6 }).map((_, index) => (
        <div
          key={index}
          className="flex flex-col items-center bg-white shadow-lg hover:shadow-xl p-6 rounded-xl hover:scale-[1.2] transition"
        >
          <h3 className="mb-4 font-semibold text-blue-700 text-lg">
            **/**/****
          </h3>
